...
def test3(cursor, conn):
    ...
    cursor.execute('''SELECT *
       FROM [math_challenges].[dbo].[questions_3]''')
    ...
    for row in rows:
        ...
        solution = factorial_n/factorial_n_minus_k
        data_to_insert.append([row.id,solution])
    insert_query = "INSERT INTO [math_challenges].[dbo].[solutions_3] values (?, ?)"
    cursor.executemany(insert_query, [(row[0], row[1]) for row in data_to_insert])
    conn.commit()
    try:
        cursor.execute('EXEC [dbo].[check_question3]')
        rows = cursor.fetchall()
