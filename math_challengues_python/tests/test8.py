...
def test(cursor, conn):
    ...
    cursor.execute('''SELECT *
       FROM [math_challenges].[dbo].[questions_8]''')
    ...
    for row in rows:
        ...
        data_to_insert.append([row.id,average])
    insert_query = "INSERT INTO [math_challenges].[dbo].[solutions_8] values (?, ?)"
    # print(data_to_insert)
    cursor.executemany(insert_query, [(row[0], row[1]) for row in data_to_insert])
    conn.commit()
    try:
        cursor.execute('EXEC [dbo].[check_question8]')
        rows = cursor.fetchall()
